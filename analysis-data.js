const ANALYSIS = {
  ...
  policy: {
    sections: [
      ...
      { id:'roadmap', title:'10. 향후 이행 계획', steps:['설계: 보안/DB 정책을 개발 규격서(Spec)에 반영 확정','구현: UID 통합 DB + 필드 암호화 + RLS + 룰 엔진 개발','테스트: 브랜드 격리 검증 + 임계값 상태 전이 테스트','배포: AS-IS 데이터 시계열 변환 + 마스터 DB 이관','사후: 법정 파기 로그 점검 + 보안 감사 정례화'] }
    ],
